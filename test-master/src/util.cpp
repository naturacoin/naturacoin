...
#else
    const char* pszModule = "naturacoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/naturacoin
    // Unix: ~/.naturacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "naturacoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "naturacoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No naturacoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override naturacoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "naturacoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong naturacoin will not work properly.");
