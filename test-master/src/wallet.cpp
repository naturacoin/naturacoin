        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-naturacoin-address
                ...
                LogPrintf("nValueIn, nValue, nFeeRet: (%lld, %lld, %lld)\n", nValueIn, nValue, nFeeRet);
                
                
                int64_t nPayFee = nTransactionFee * (1 + (int64_t)nBytes / 1000);
                bool fAllowFree = AllowFree(dPriority);
                int64_t nMinFee = GetMinSendFee(nValue); //1% //GetMinFee(wtxNew, nBytes, fAllowFree, GMF_SEND);
                LogPrintf("%lld = GetMinFee(%lld, %d)\n", nMinFee, nBytes, fAllowFree);
