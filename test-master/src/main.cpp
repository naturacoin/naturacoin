...
// Copyright (c)      2014 The naturacoin developers
...
#if defined(NDEBUG)
# error "naturacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "naturacoin Signed Message:\n";
...
{    
    ...
    if(t > PercentageFeeRelayBegin || (t > PercentageFeeSendingBegin && mode==GMF_SEND) )  
    {
        ...
        */
        // naturacoin percentage fee implementation
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
...
{
    //100k coins total. 20000 premine, 10 per block after
    ...
    int firstreward=adjust+(8*HOUR*DAY); //first 7 days of IPO
    int secondreward=firstreward+(7*HOUR*DAY); //next 7 days of mining

    int64_t sub=0;
    if(nHeight==1)
    {
        sub=20000*reward; //premine of 20000 coins, 20% of cap
    }
    ...
    {
        sub=reward*8; //normal phase of 8 coins
    }
    else
    {
        sub=0; //no coins
...
static const int64_t nTargetTimespan = 60 ; // naturacoin: every 1 minute
static const int64_t nTargetSpacing = 60; // naturacoin: 1 minute
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // naturacoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    //naturacoin slingshield modification:
    /*intentions:
    ...
    */
    int64_t adjust=0;
    if(pindexLast->nHeight > (HOUR*DAY*15)) //don't activate til rewards drop
    {
        const static int64_t stepcount=12;
        //min fees: 0.1, 0.2, etc etc corresponding to 1% increase, 2% increase etc
        const static int64_t steps[stepcount]={1*COIN,2*COIN,3*COIN,4*COIN,5*COIN,6*COIN,7*COIN,8*COIN,9*COIN,10*COIN,11*COIN,21*COIN};
...
void ThreadScriptCheck() {
    RenameThread("naturacoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in naturacoin
...
{
    // naturacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
